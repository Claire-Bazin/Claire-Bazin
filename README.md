# **$\color{teal}{\text{Claire BAZIN - Ingénieure de Recherche}}$**

### 📊 Expertise : Gestion de Données & Coordination de Recherche
Ingénieure diplômée de l'**ENSCBP**, mon pacours m'a permis d'aborder les enjeux techniques de la filière viticole et de me spécialiser dans la gestion de protocoles de recherche. Je souhaite mettre cette rigueur et mes compétences en strcturation de données au service du Living Lab.

---

### 🛠 Compétences
#### **Gestion de données et méthodologie**
- **Conception sur REDCap :** Création de projets de recherche (définition des variables, paramétrage des formulaires de saisie et gestion des contrôles de cohérence).
- **Qualité des données :** Expérience de la saisie de données sur diverses plateformes et expériences du circuit de correction et du contrôle de cohérence des données pour garantir des bases exploitables.
- **Rigueur opérationnelle :** Mise en place de procédures de recueil pour assurer la traçabilité et la fiabilité des résultats de terrain.

#### **Outils informatiques et statistiques**
- **Logiciel R :** Traitement et analyse statistique de données
- **Pack Office :** Maîtrise d'Excel (tableaux de suivi), Word et PowerPoint

#### **Relationnelles**
- Travail en équipe pluridisciplinaire et interaction avec différents services
- Autonomie, rigueur et capacité d'adaptation

#### **Langues**
- Anglais professionnel : niveau B2 (TOEIC, score de 830)

---

### 🧪 Expériences professionnelles
#### **Attachée de Recherche Clinique**
*CHU de Bordeaux (Pessac) / Sept. 2023 - En cours / CDD*
- **Pilotage de protocoles :** Coordination et suivi d'essais en environement réglementé. Garante de la conformité et de l'éthique des processus.
- **Data Management :** Vérification de la cohérence, traçabilité et sécurisation de données.
- **Qualité :** Rédaction de procédures opérationnelles, de documents de synthèse et gestion logistique des essais.
- **Interface partenaires :** Liaison entre les équipes médicales et les promoteurs.

#### **Professeure suppléante**
*Collège - Lycée Sainte Marie Bastide (Bordeaux) / Fév. - Mars 2023 / CDD*
- Préparation et animation de cours, d'activités et de TP en SVT (classe de la 4ème à la 1ère).
- Organisation, planification et gestion de groupes.
- Développement de capacités de communication, d'adaptation et de formation.

#### **Ingénieure en biologie**
*IDPOC (Mulhouse) / Nov. 2022 - Jan. 2023 / CDD*
- Suivi de procédés biologiques en bioréacteurs et surveillance des paramètres de production.
- Réalisation d'analyses et de contrôles qualité des biomasses selon des procédures établies.
- Réalisation d'une veille scientifique et règlementaire, identification des allégations nutritionnelles et santé.

#### **Assistante R&D**
*Synutra France International(Rennes) / Fév. - Août 2022 / Stage*
- Optimisation d'un procédé industriel par électrodialyse avec mise en place d'essais à l'échelle pilote.
- Rédaction de protocoles expérimentaux et de rapports. Analyse et interprétation de résultats expérimentaux.
- Collaboration avec des équipes de l'INRAE pour l'interprétation et la validation des résultats.
- Participation à la veille technologique et scientifique.

#### **Agent spécial de la Résilience**
*Département de la Gironde (Bordeaux) / Mai - Sept. 2021 / Service civique*
- Recensement d'initiatives citoyennes sur l'alimentation.
- Réalisation de questionnaires et analyse des données pour des actions de sensibilisation.

#### **Ingénieure assistante Qualité**
*Domaine de Chevalier (Léognan) / Avril - Sept. 2020 / Stage*
- Contrôle qualité des mises en bouteilles et élaboration de protocoles d'optimisation pour diminuer l'oxygène dissous dans le vin.
- Rédaction de protocoles et participation à l'amélioration continue des pratiques.
- Analyses statistiques des résultats (R Studio) et présentation des conclusions aux équipes.

#### **Ingénieure assistante R&D microbiologie**
*BioLaffort (Floirac) / Juin - Nov. 2019 / Stage*
- Mise en place d'un plan d'expérience pour la sélection et la caractérisation de levures.
- Recherches bibliographiques ; Rédaction de protocoles et de rapports.
- Analyse statistique des résultats (R Studio) ; Présentation de l'anvancée du projet.
- Représentation de l'entrerpise chez le client.

#### **Assistante de laboratoire de recherche**
*INRAE - UFR Biologie et Pathologie du Fruit (Villenave d'Ornon) / Juin - Juil. 2017 / Stage*
- Transformation de bactéries pour la production de protéines.
- Tenue du cahier de laboratoire.

---

### 🎓 Formations
#### **Diplôme d'Attachée de Recherche Clinique**
*2023 - ISVM (Bordeaux)*

#### **Master 2, Plantes à Valeur Santé et Biomolécules d'Intérêt**
*2022 - Université de Bordeaux*
- Projet d'audit d'un complément alimentaire et élaboration d'un protocole d'étude clinique.
- Rédaction d'une revue scientifique en anglais.

#### **Diplôme d'Ingénieure en Agroalimentaire - Génie biologique, spécialité Conception et Production en Industrie**
*2020 - ENSCP (Bordeaux INP)*
- Projet Recherche, Développement et Innovation : Etude, Formulation et Conception d'un produit alimentaire.
- Réalisation d'un audit interne mené sur l'atelier boulangerie d'une grande surface sur le référentiel des PMS.

#### **DEUG de Sciences, Technologies, Santé, mention Science de le Vie**
*2017 - Université de Bordeaux - Mention Bien*

---

### 📫 Contact
- [Mon profil LinkedIn](https://www.linkedin.com/in/claire-bazin-072ba018b/)

---
*Profil créé pour le concours d'Ingénieur de Recherche INRAE - Living Lab BACCHUS.*
